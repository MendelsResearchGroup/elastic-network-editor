...
export default function App() {
  ...
  const scale = zoomPercent / 100; // 1.0 = 100%
  ...
  const addBond = () => {
    ...
    setBonds([...bonds, { id, i: atoms[0].id, j: atoms[1].id, k: 1 }]);
  };
  ...
  return (
    <div className="w-full h-full grid grid-cols-1 xl:grid-cols-[1fr_480px] gap-4 p-4 bg-white text-slate-900">
      <div className="flex flex-col gap-3">
        ...
        <CanvasView atoms={atoms} bonds={bonds}  scale={scale} selected={selected} setSelected={setSelected} setAtoms={setAtoms} setBonds={setBonds}/>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="rounded-2xl border p-3 grid grid-cols-2 gap-2">
          ...
          <button className="px-3 py-1.5 rounded-xl bg-slate-900 text-white" onClick={() => { setBonds([]); }}>Clear bonds/angles</button>
        </div>
      </div>
    </div>
  );
